...
Inductive substi (s:tm) (x:id) : tm -> tm -> Prop := 
  | s_var1 : 
      substi s x (tvar x) s
  | s_var2 :
      forall x', x <> x' -> substi s x (tvar x') (tvar x')
  | s_abs1 :
      forall T t1, substi s x (tabs x T t1) (tabs x T t1)
  | s_abs2 :
      forall x' T t1 t1', x <> y -> substi s x t1 t1' -> substi s x (tabs x T t1) (tabs x' T t1')
  | s_app :
      forall t1 t1' t2 t2', substi s x t1 t1' -> substi s x t2 t2' -> substi s x (tapp t1 t2) (tapp t1' t2')
  | s_true : 
  ...
  | s_if :
      ...
      substi s x t1 t1' -> substi s x t2 t2' -> substi s x t3 t3' -> substi s x (tif t1 t2 t3) (tif t1' t2' t3')
.
