...
Case "<-".
    ...
    simpl in H5.
    SearchAbout negb.
    rewrite negb_involutive_reverse with (b:= beval st b).
    rewrite H5.
    reflexivity.
    assumption.
    apply E_IfTrue.
...
Case "->".
    ...
    inversion H0.
Case "<-".
    apply WHILE_true_nonterm with (c:=SKIP) (st:=st) (st':=st') in H0.
    inversion H0.
    unfold bequiv.
    simpl.
    reflexivity.
...
assert (st' = (update st' X (aeval st' e))).
  apply functional_extensionality. intros. rewrite <- H.
  rewrite update_same. reflexivity. reflexivity.
  rewrite H1 at 2. apply E_Ass. reflexivity.
  inversion H0; subst. rewrite <- H.
  replace (update st X (aeval st (AId X))) with st.
  apply E_Skip. apply functional_extensionality.
...
Proof.
  intros a1 a2 H. unfold aequiv. intros st. symmetry. apply H.  Qed.
...
Proof.
 ...
 (* assert (c1 / st || st' <-> c2 / st || st') as H'. 
    SCase "Proof of assertion". apply H. assumption.*)
Qed.
...
Example test_optimize_0plus_aexp:
  optimize_0plus_aexp (APlus (ANum 2)
                        (APlus (ANum 0)
                               (APlus (ANum 0) (ANum 1))))
  = APlus (ANum 2) (ANum 1).
Proof. reflexivity. Qed.
...
unfold atrans_sound. intros a. unfold aequiv. intros st.
  aexp_cases (induction a) Case; simpl;
    (* ANum and AId follow immediately *)
    try reflexivity;
    (* AMinus, and AMult follow from the IH *)       
...
Proof.
  ...
  Case "BLe". 
    ...
    (*Stupid coq does not allow you to rewrite at 2. It keeps looping at a1 *)
    replace (aeval st a2) with (aeval st (optimize_0plus_aexp a2)) by
...
Definition combined_optimizer (c : com) : com :=
optimize_0plus_com (fold_constants_com c).

(*Example test_combined_optimizer: 
    combined_optimizer
    (X ::= APlus (ANum 4) (ANum 5);;
     Y ::= AMinus (AId X) (ANum 3);;
     IFB BEq (AMinus (AId X) (AId Y)) (APlus (ANum 0) (ANum 4)) THEN
       SKIP 
     ELSE
       Y ::= ANum 0
     FI). 
     ...
     (X ::= ANum 9;;
     Y ::= AMinus (AId X) (ANum 3);;
     IFB BEq (AMinus (AId X) (AId Y)) (ANum 4)) THEN
       SKIP 
     ELSE
       Y ::= ANum 0
     FI).
...
Theorem combined_optimizer_sound : 
  ctrans_sound combined_optimizer.
Proof.
  unfold ctrans_sound. intros.
  unfold combined_optimizer.
...
Inductive ceval : com -> state -> state -> Prop :=
  ...
  | E_WhileLoop : forall (st st' st'' : state) (b1 : bexp) (c1 : com),
                  ...
                  (WHILE b1 DO c1 END) / st || st''
  | E_Havoc : forall (st : state) (i:id) (n : nat), (HAVOC i) / st || update st i n
...
Proof. apply E_Seq with (st':=empty_state). apply E_Skip. apply E_Havoc. Qed.
(** [] *)
...
inversion H2; subst.
inversion H5; subst.
apply E_Seq with (st:=st) (st':=update st Y n0).
apply E_Havoc.
...
inversion H2; subst.
inversion H5; subst.
apply E_Seq with (st:=st) (st':=update st X n0).
apply E_Havoc.
...
Proof.
right.
unfold not, cequiv, ptwice, pcopy.
Admitted.
(** [] *)
