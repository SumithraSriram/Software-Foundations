...
(** Which of the following Hoare triples are _valid_ -- i.e., the
   ...
   5) {{True}} SKIP {{False}} - Not valid

   6) {{False}} SKIP {{True}} - Valid

   7) {{True}} WHILE True DO SKIP END {{False}} - Valid (infinite)
   ...
   9) {{X = 1}}
      WHILE X <> 0 DO X ::= X + 1 END - Valid (infinite)
      {{X = 100}}
...
(*If the post condition is true, no matter what the pre-condition is, the triple 
is always true *)
...
(*If the pre condition is false, no matter what the post-condition is, the triple 
is always true *)
...
(** The assignment rule looks backward to almost everyone the first
    ...
    seemingly natural one:
      ------------------------------ (hoare_asgn_wrong)
      {{ True }} X ::= a {{ X = a }}
      Doesn't hold for an empty state.
