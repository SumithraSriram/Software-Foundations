...
Check x. baz
Check y. baz
Check x y true. Error.
Check x y _ true. baz
Check x _. baz
Check y x _ true. Error.

 
This inductive type has no elements. This is because all the types in baz are inductive,
which will always lead to an infinite string of baz's. There is no base case in the definition
that will help stop induction of the type.
...
Check @combine.
Check combine.
Check combine [1;2;3] [true;true;true].
Eval compute in (combine [1;2] [false;false;true;true]).
...
match l with
| [] => ([], [])
| xy :: txy => match xy with 
               |(x,y) => ((x :: fst (split txy)) , (y :: snd (split txy)))
               end
end.
...
Fixpoint lastp {X:Type} (l:list X) : option' :=
match l with
| nil => None'
| [x] => Some' x
| h :: t => lastp (t)
...
Definition lastp' {X:Type} (l:list X) : option' :=
match (rev l) with
| nil => None'
| h :: t => Some' h
end.

Example l1 : lastp [1;2;3;4] = Some' 4.
Proof. simpl. reflexivity. Qed.

Example l2 : lastp' [1;2;3;4] = Some' 4.
Proof. simpl. reflexivity. Qed.
...
(*List with mixed elements of 2 types*)
Inductive listxy {X Y:Type} : Type:=
|nilxy : listxy
|consx : X -> listxy -> listxy
|consy : Y -> listxy -> listxy.

Check consy 2 (consy 1 (consx false (consy 4 (nilxy)))).
...
Definition filter_even_gt7 (l : list nat) : list nat :=
  filter (fun y => blt_nat 7 y) (filter (fun x => evenb x) l).
...
(*Factorial of a number using fold*)
...
fold mult (createlist n) 1.

Example f1 : fact' 1 = 1.
Proof. reflexivity. Qed.

Example f2 : fact' 5 = 120.
Proof. reflexivity. Qed.
(*****)
...
Example constfun_example3 : (constfun true) 99 = true.
Proof. reflexivity. Qed.
...
Example override_example01 : (override (constfun false) 0 true) 5= false.
Proof. reflexivity. Qed.
...
Definition fold_map {X Y:Type} (f : X -> Y) (l : list X) : list Y :=
 fold (fun (k:X) (y:list Y) => f k :: y ) l [].


Definition fold_filter {X:Type} (f: X -> bool) (l :list X) : list X :=
fold (fun (k:X) (t:list X) => if (f k) then k :: t
                                       else t ) l [].

Example fold_filter1: fold_filter evenb [1;2;3;4] = [2;4].
Proof. reflexivity.  Qed.
...
Definition lastp' (l:natlist) : natoption :=
match (rev l) with
| nil => None
| h :: t => Some h
end.
