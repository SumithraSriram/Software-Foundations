...
(*
There exists a number n, such that itssucessor is beautiful.
*)
...
Lemma excluded_middle_nat: forall (X:Type) (x:X) (P : X -> Prop),
excluded_middle -> P x \/ ~ P x.
Proof.
intros.
...
|nil : all P []
|next : forall (l:list X) (x:X), P x -> all P l -> all P (x::l).
...
|all_nil : in_order_merge [] [] []
|checkl1 : forall x l1 l2 l, in_order_merge l1 l2 l -> in_order_merge (x::l1) l2 (x::l) 
|checkl2 : forall x l1 l2 l, in_order_merge l1 l2 l -> in_order_merge l1 (x::l2) (x::l).



Theorem filter_challenge:forall (X:Type) (P: X->bool) (l1 l2 l: list X),
in_order_merge l1 l2 l -> forallb P l1 = true -> forallb (fun x => negb(P x)) l2 =true ->
filter P l = l1.
Proof.
intros.
induction H.
...
destruct H3.
simpl.
assert (A: negb (P x) = true -> P x = false).
intros.
destruct (P x).
inversion H4.
reflexivity.
...
Lemma nilapp : forall (X:Type) (t:list X), t ++ [] = t.
Proof.
intros.
induction t.
reflexivity.
...
Definition disjoint {X:Type} (l1 l2: list X): Prop :=
 forall x,  not(appears_in x l1 /\ appears_in x l2).
...
|rnil : no_repeats []
|rnr : forall x l,  not(appears_in x l) -> no_repeats l -> no_repeats (x::l).
...
|snil : nostutter []
|s1 : forall x, nostutter [x]
|sns : forall  x y l,  x <> y -> nostutter (y::l)-> nostutter (x::y::l).
...
Proof. repeat (apply sns); repeat (apply beq_nat_false; reflexivity). apply s1. Qed.
(* 
...
Theorem app_add: forall (X:Type) (l1:list X) (x:X), x::l1 = [x]++l1.
Proof.
reflexivity.
Qed.
(*
...
Proof.
   ...
   apply r_here. apply H2. apply r_later.
   ...
   unfold lt. rewrite app_length. apply H1.
   apply contradiction_implies_anything with (P:=(appears_in x l2)).
   split. apply H0. apply ai_here. apply H3.
Qed.
*)
Theorem pigeonhole_principle: forall (X:Type) (l1  l2:list X), 
   excluded_middle -> 
...
simpl in H1.
unfold excluded_middle in H.
destruct H with (P:= appears_in x l1').
...
Theorem pigeonhole_principle_strong: forall (X:Type) (l1  l2:list X),  
   (forall x, appears_in x l1 -> appears_in x l2) -> 
   length l2 < length l1 -> 
   repeats l1.  
Proof.
...
apply H1.
simpl in H0.
apply appears_in_app_split with (x:=x) in H.
inversion H; subst.
inversion H1; subst.
Abort.
