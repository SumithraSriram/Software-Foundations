...
try(simpl;rewrite ?optimize_0plus_sound''';reflexivity).
...
Inductive bevalR: bexp -> bool -> Prop:=
  ...
  | E_BEq : forall (a1 a2: aexp) (n1 n2 : nat),
      (a1 || n1) -> (a2 || n2) -> (BEq a1 a2) ||b beq_nat n1 n2
  | E_BLe : forall (a1 a2: aexp) (n1 n2 : nat),
      (a1 || n1) -> (a2 || n2) -> (BLe a1 a2) ||b ble_nat n1 n2
...
Case "->".
   intros.
   induction H; simpl; try(simpl; constructor);
...
rewrite neq_id.
reflexivity.
unfold not;intros;unfold not in n;apply n;symmetry; apply H.
unfold not;intros;unfold not in n;apply n;symmetry; apply H.
unfold not;intros;unfold not in n;apply n;symmetry; apply H. *)
Qed.
...
|skipnw : no_whilesR SKIP
|assnw : forall id x, no_whilesR (id ::= x)
|seqnw : forall c1 c2, no_whilesR c1 -> no_whilesR c2 -> no_whilesR (c1;;c2)
|ifnw : forall b c1 c2, no_whilesR(c1) -> no_whilesR(c2) -> no_whilesR (IFB b THEN c1 ELSE c2 FI).
...
split.
intros.
induction c; try(simpl in H); 
try(inversion H; apply andb_true_iff in H1; destruct H1);
try (constructor);
try (apply IHc1; assumption);
try (apply IHc2; assumption).
...
Case "SKIP".
    ...
    constructor.
Case "ASS".
    exists (update st id (aeval st x)).
    constructor.
    reflexivity.
Case "SEQ".
    destruct IHno_whilesR1 with st.
...
| SLoad i :: t => s_execute st (st i::stack) t
| SPlus :: t => match stack with
                |x :: y :: ts => s_execute st (y+x :: ts) t
...
Theorem s_execute_append : forall (e1 e2 : list sinstr) (st : state)  (s : list nat),
  s_execute st s (e1 ++ e2) = s_execute st (s_execute st s e1) e2.
Proof.
intros e1.
induction e1.
simpl.
...
reflexivity.
intros.
destruct a; destruct s; try(apply IHe1); try (reflexivity);
  destruct s; try(apply IHe1); simpl; try (reflexivity).
Qed.

Theorem s_execute_aeval : forall (e : aexp) (st : state)  (s: list nat),
  s_execute st s (s_compile e) = aeval st e :: s.
Proof.
intros e.
induction e; intros; try (reflexivity);
try (simpl; rewrite s_execute_append; 
rewrite IHe1; rewrite s_execute_append;
...
try (reflexivity);
simpl;
try(rewrite s_execute_append;
rewrite IHe1;
rewrite s_execute_append;
rewrite s_execute_aeval;
reflexivity).
Qed.
