...
(** Briefly explain the difference between the tactics [apply] and
    ...
    applied?
  (* FILL IN HERE *)
Rewrite only replaces a part/whole of the expression using the theorem or lemma mentioned. Apply however can
be used only when the expression to be proved completely matches an existing theorem/lemma. Applying that theorem
...
Proof.
  ...
  Case "l = v' :: l'". 
    ...
    SCase "n = S n'".
      apply f_equal. apply IHl'. simpl in eq. inversion eq. reflexivity. Qed.
...
(** Note: Whenever you perform induction/destruct on a pair, destruct it as [x y] txy, instead of xy txy.
...
intros.
unfold override.
destruct (beq_nat k1 k2) eqn:bH.
SearchAbout beq_nat.
apply beq_nat_true in bH.
rewrite <- bH.
symmetry.
apply H.
reflexivity.
...
length l1 = length l1 -> split (combine l1 l2) = (l1,l2).
...
match l with
| [] => true
| x :: tx => if (test x) then (forallb test tx)
                         else false
end.
...
match l with
| [] => false
| x :: tx => if (test x) then true
                       else existsb test tx
end.
