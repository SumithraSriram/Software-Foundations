...
Definition d' (n:nat) : nat:= 2 *n.

(** Use induction to prove this simple fact about [double]: *)
Lemma double_plus' : forall n, d' n = n + n .
Proof.  
intros n.
destruct n.
unfold d'.
...
(* FILL IN HERE *)
Destruct: It only seperates the given variable into multiple cases as mentioned in the statement.
Induction: It, apart from doing the job of destruct, also initialises the inductive hypothysis, 
which can later be used in the inductive step to prove the theorem.
*)
(** [] *)
...
Theorem mult_0 : forall m :nat, m * 0 = 0.
Proof.
intros m.
induction m as[|m1].
Case "m=0".
...
Theorem mult_next : forall n m:nat, 
m * S n = m + m * n.
Proof.
intros n m.
induction m as [|m1].
...
Theorem bin_add_0 : forall n:bin,
...
Theorem Sn_eq_plus1 : forall n:nat, S n = n +1.
Proof.
intros n.
induction n.
reflexivity.
...
Example test_bin:nat_to_bin 0 = Ob.
Proof. simpl. reflexivity. Qed. 
Eval compute in (nat_to_bin 3).
Eval compute in (nat_to_bin (bin_to_nat (S2n1 (S2n Ob)))).

Theorem proof_correctness : forall n:nat, bin_to_nat(nat_to_bin(n)) = n.
Proof.
intros n.
induction n as [|n'].
Case "n=0".
...
(** The theorem nat_to_bin(bin_to_nat(n)), where n is of type bin, does not work
as any input can be defined with any number of (S2n Ob) whcih will essentially give
zero. FOr example, S2n1 Ob = S2n1 (S2n Ob) = S2n1 ( S2n (S1n Ob)) and so on. Though
they represent the same natural number, they can be represented in infinite number of
ways as shown. Hence, when an input is given in bin form and converted to natural number
and then bin again, the final answer got is the most simplified form of bin, and not
what was given as input. *)

(**The following function is used to find out if it is just a bunch of S2n's with an Ob
at the end *)

(**Fixpoint S2nend (b:bin) :(bool) :=
match b with
|Ob => true
|S2n n' => S2nend(n')
|S2n1 n' =>false
...
match b with
|Ob => Ob
|S2n b' => match (normalize b') with
           | Ob => Ob
           | b' => S2n b'
           end
|S2n1 b' => S2n1 (normalize (b'))
end.

Eval compute in (nat_to_bin(bin_to_nat (S2n (S2n1 (S2n (S2n1 (S2n (S2n Ob)))))))).
Eval compute in normalize(S2n (S2n1 (S2n (S2n1 (S2n (S2n (S2n Ob))))))).

Lemma bin_add : forall b: bin,
(bin_to_nat b + bin_to_nat b) = bin_to_nat(S2n b).
Proof.
Admitted.

Lemma dbl_S : forall n :nat,
nat_to_bin (n + n) = S2n (nat_to_bin (n)).
Proof.
Admitted.

Theorem normalize_correct : forall b:bin, 
(nat_to_bin(bin_to_nat(normalize(b)))) = normalize(b).
Proof.
intros b.
induction (normalize b) as [|b1|b2] eqn:dnor.
Case "nb=0".
simpl.
reflexivity.
Case "nb=b1".
